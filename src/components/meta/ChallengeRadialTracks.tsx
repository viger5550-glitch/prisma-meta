...
export function ChallengeRadialTracks() {
  const radiusPx = 260;
  const cards = mapChallengeTracksToRadialItems(CHALLENGE_TRACKS);
    ...
    <section className="bg-[#F2F2F2]">
      <div className="mx-auto box-border w-full max-w-[1280px] px-6 py-16 md:px-12 md:py-24 lg:px-[96px] lg:py-[160px]">
          ...
          renderCard={(item, index) => {
            ...
            return (
              <div className={hoverStyles.scaleWrap}>
                <article
                  className={`box-border flex h-[230px] w-[160px] flex-col justify-between rounded-[2px] p-5 ${isDark ? "bg-black text-white shadow-[0_8px_28px_rgba(0,0,0,0.18)]" : "border border-black/[0.06] bg-white text-black shadow-[0_8px_28px_rgba(0,0,0,0.08)]"}`}
                >
        ...
        <div className="grid grid-cols-1 gap-6 lg:hidden">
          {CHALLENGE_TRACKS.map((track, index) => {
            const isDark = index % 2 === 0;
            return (
              <article
                key={`m-${track.slug}`}
                className={`box-border flex min-h-[220px] flex-col justify-between rounded-[2px] p-6 ${isDark ? "bg-black text-white shadow-[0_8px_28px_rgba(0,0,0,0.18)]" : "border border-black/[0.06] bg-white text-black shadow-[0_8px_28px_rgba(0,0,0,0.08)]"}`}
              >
                <div>
                  <p className={`text-[10px] font-medium uppercase leading-[1.25] tracking-[0.16em] ${interSans} ${isDark ? "text-white/85" : "text-zinc-500"}`}>
                    {track.label}
                  </p>
